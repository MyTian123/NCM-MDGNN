...
class icosphere(object):
    def __init__(self, level=0, upward=False):
        ...
        for l in range(self.level):
            self.subdivide()
            self.normalize()
        self.lat, self.long, self.lat2, self.long2 = self.xyz2latlong()
        self.xx, self.yy, self.zz = self.xyzout()
    ...
    def icosahedron(self, upward=True): #构建初始20面体
        """
        ...
        #  1.90是为了让点间距缩小到1
        faces = np.reshape(faces, (-1, 3))  #哪三个顶点组成了面
        if upward:
    ...
    def xyz2latlong(self):
        ...
        long2 = long #* 180 /np.pi
        lat2 = lat #* 180 / np.pi
        return lat, long, lat2, long2
    ...
    def _upward(self, V_ico, F_ico, ind=11):
        V0 = V_ico[ind]
        Z0 = np.array([0, 0, 1])
        k = np.cross(V0, Z0) #计算两个向量（向量数组）的叉乘
        ct = np.dot(V0, Z0) #计算两个向量（向量数组）的点乘
        st = -np.linalg.norm(k)
