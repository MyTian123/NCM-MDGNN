...
def test(args, model, test_loader, device, logger, forecast_len):
    ...
    with torch.no_grad():
        for data, label in tqdm(test_loader):
            ...
            days = int(forecast_len/24) #args.forecast_len
            for iidx in range(days):
                data = model(data) #输入数据直接推理
            output = data
            ...
            Rmse_q500 = 0
            for batches in range(label.shape[0]):
                if label[batches,-1,1,100,100]!=0:
                    climatology = np.array(label[batches,6,1])
                    out_temp = out_temp_acc[batches,1,1]*(std_[1,15])+mean_[1,15] - climatology #(output.clone().detach().cpu())[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]   
                    LLabel = np.array(label[batches,1,1]*(std_[1,15])+mean_[1,15]) - climatology #(label[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]) - climatology
                    acc_cli = acc(out_temp,LLabel)
                    Rmse_q500 += np.sqrt(mean_squared_error((out_temp+climatology).flatten(), (LLabel+climatology).flatten()))
                    # print(acc_cli)
                    ACC += acc_cli.mean()
                    Con_+=1
            if Con_>0:
            ...
            for batches in range(label.shape[0]):
                if label[batches,-1,1,100,100]!=0:
                    climatology = np.array(label[batches,7,1])
                    out_temp = out_temp_acc[batches,2,1]*(std_[2,15])+mean_[2,15] - climatology #(output.clone().detach().cpu())[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]   
                    LLabel = np.array(label[batches,2,1]*(std_[2,15])+mean_[2,15]) - climatology #(label[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]) - climatology
                    acc_cli = acc(out_temp,LLabel)
                    Rmse_t500 += np.sqrt(mean_squared_error((out_temp+climatology).flatten(), (LLabel+climatology).flatten()))
                    # print(acc_cli)
                    ACC += acc_cli.mean()
                    Con_+=1
            if Con_>0:
            ...
            for batches in range(label.shape[0]):
                if label[batches,-1,1,100,100]!=0:
                    climatology = np.array(label[batches,8,1])
                    out_temp = out_temp_acc[batches,3,1]*(std_[3,15])+mean_[3,15] - climatology #(output.clone().detach().cpu())[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]   
                    LLabel = np.array(label[batches,3,1]*(std_[3,15])+mean_[3,15]) - climatology #(label[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]) - climatology
                    acc_cli = acc(out_temp,LLabel)
                    Rmse_u500 += np.sqrt(mean_squared_error((out_temp+climatology).flatten(), (LLabel+climatology).flatten()))
                    # print(acc_cli)
                    ACC += acc_cli.mean()
                    Con_+=1
            if Con_>0:
            ...
            for batches in range(label.shape[0]):
                if label[batches,-1,1,100,100]!=0:
                    climatology = np.array(label[batches,9,1])
                    out_temp = out_temp_acc[batches,4,1]*(std_[4,15])+mean_[4,15] - climatology #(output.clone().detach().cpu())[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]   
                    LLabel = np.array(label[batches,4,1]*(std_[4,15])+mean_[4,15]) - climatology #(label[batches,0,0]*std_[0,15][...,None]+mean_[0,15][...,None,None]) - climatology
                    acc_cli = acc(out_temp,LLabel)
                    Rmse_v500 += np.sqrt(mean_squared_error((out_temp+climatology).flatten(), (LLabel+climatology).flatten()))
                    ACC += acc_cli.mean()
                    Con_+=1
            if Con_>0:
                ACC /= Con_
    ...
    Acc_z500 = np.mean(acc_set0)
    Acc_Q500 = np.mean(acc_set1)
    Acc_T500 = np.mean(acc_set2)
    Acc_U500 = np.mean(acc_set3)
    Acc_V500 = np.mean(acc_set4)

    print('ACC of z500 =',Acc_z500)
    print('ACC of q500 =',Acc_Q500)
    print('ACC of t500 =',Acc_T500)
    print('ACC of u500 =',Acc_U500)
    print('ACC of v500 =',Acc_V500)

    print('RMSE of z500 =',Rmse_z500)
    print('RMSE of q500 =',Rmse_q500)
    print('RMSE of t500 =',Rmse_t500)
    print('RMSE of u500 =',Rmse_u500)
    print('RMSE of v500 =',Rmse_v500)
...
def main(forecast_len):
    ...
    trainset = ClimateSegLoader(args.data_folder,6, forecast_len, args.climatology_dir, mean_, std_, "train")
    valset = ClimateSegLoader(args.data_folder,6, forecast_len, args.climatology_dir, mean_, std_, "val")
    ...
    model = Multi_uunet(fl=forecast_len, bs=args.batch_size, mesh_folder=args.mesh_folder, in_ch=5, out_ch=5, max_level=args.max_level, min_level=args.min_level, fdim=args.feat) #加入bs和fl，用bs代表输入天数（batch size），用fl代表预测时
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        rmse,CORR,Acc_z500 = test(args, model, val_loader, device, logger, forecast_len)
...
if __name__ == "__main__":
    for forecast_len in range(10):
        print('第',(forecast_len+1),'天预测结果 \n')
        forecast_len = int(forecast_len*24+24)
        main(forecast_len)
